# Krosis Changelog

## 1.1.1

Fix to skyparkour not working

## 1.1.0

Rebalance of the combat system, stagger rework. Loot overhaul, and countless bugfixes, mod updates etc.

- **Added:** 30 mods
- **Removed:** 59 mods
- **Updated:** 0 mods

## Added Mods

- ---KROSIS v1.1---_separator
- A-Pose Bug Fix - Universal Behavior Runtime
- AW High Poly Female Replacers All in One
- dMenu NG
- Faithful Faces - NPC Overhaul with SMP Hair Physics
- Krosis Custom Patches
- Krosis NPC Output
- KROSIS RUS
- Lux (main plugin update)
- MAW Assets
- MaxsuPoise
- Men of Skyrim Refined Resources
- Modern Lighting Overhaul 2
- Modern Stagger Lock
- Monster Mod AE - Fluffworks Patch
- Oathvein RaceMenu DIP Patch
- Oathvein UI
- Open World Loot - Creation Club Integration
- Open World Loot - Encounter Zone and Loot Overhaul
- Open World Loot - Jaysus Swords SE Patch
- OWL - CC Crossbows
- OWL - Summermyst Patch
- RnR - Attack Speed Fix
- Rogues 'n Raiders
- Save & Load Accelerator for SKSE Cosaves (S.L.A.C.K.)
- Silent Horizons 2 - Universal Core
- Smart Talk (Dialogue Menu Enhancer)
- Survival Mode Improved - SKSE
- True Sons of Skyrim Refined Resources
- WHEELER - Refined

## Removed Mods

- (NEW) Thogra gra-Mugur - Orc Follower and Quest
- (NEW) Thogra gra-Mugur - Orc Follower and Quest Exterior Camp
- ---KROSIS v1.0---_separator
- Amon ENB
- Chocolate Poise
- Chocolate Poise Plus - Addons and Rebalancing
- ConflictDeleter_Backup_2025_08_18_01_36_04
- Creation Club - Misc Patches
- Draugr MCO-DXP
- Edge UI
- Edge UI - Even More Misalignment Fixes
- Edge UI - Explorer Addon
- Edge UI - Explorer Addon - Patch Collection
- Edge UI - HUD Preset
- Edge UI Explorer Addon - Racemenu DIP
- eeekie's Thogra
- ENB Helper SE
- Falkreath Gravestone Diversity - Base Object Swapper
- Girls of Dawnstar Npc Replacer SkyPatcher
- Girls of Falkreath Npc Replacers SkyPatcher
- Girls of Markarth Npc Replacer SkyPatcher
- Girls of Morthal Npc Replacer
- Girls of Riften Npc Replacer SkyPatcher
- Girls of Small Towns and Around Skyrim Npc replacer
- Girls of Solitude Npc Replacer SkyPatcher
- Girls of Whiterun Npc Replacers SkyPatcher
- Girls of Windhelm Npc Replacer SkyPatcher
- Girls of Winterhold Npc Replacer SkyPatcher
- Heritage 2 - WACCF Compatibility
- JellyFishInLoop SkyParkour animations
- Just Bite
- Krosis Conflicts
- Krosis NPC Merge
- Men of Skyrim Refined - JK Castle Dour Patch
- MoS EasyNPC Teeth Workaround
- New Horizons UI (Font Only)
- Pandora Output Legacy
- Plus Combat
- RaceMenu - Edge UI Patch (DIP)
- Skyrim Save System Overhaul 3 (SSSO 3)
- Snazzy Aldmeri Cultured Furniture - WACCF Patch
- Snazzy Common Cupboards Dressers and End Tables - WACCF Patch
- Snazzy Common Wardrobes - WACCF Patch
- Snazzy Noble Cupboards - WACCF Patch
- Snazzy Orc Dressers and End Tables -WACCF Patch
- Snazzy Upper Cupboards Dressers and End Tables - WACCF Patch
- SSE Engine Fixes (skse64 plugin)
- SSE Engine Fixes - Complete Config
- SSE-AT Output
- SSEEdit Output
- Stuck on Sleeper Fix - Exit Noble Bed Double 02
- Sure of Stealing
- Survival Mode Prompt Removed
- Synthesis Output
- True Sons Of Skyrim Refined EasyNPC Fix
- TSoS EasyNPC Teeth Workaround
- Vanaheimr Landscapes - Lux Patch
- Weapons Armor Clothing and Clutter Fixes
- Wheeler CTD-Fix

## Updated Mods

No mods updated.

## 1.0.2

Updated some mods, removed useless things, reduced overall size of installed modlist. Updated Pandora to 4.0.3, fixed missing textures. Should be safe to update mid-game, but i recommend you to update in interior cell.

- **Added:** 8 mods
- **Removed:** 6 mods
- **Updated:** 0 mods

### Added Mods

- Daedric Enigma - Dusk -
- Disable Draugr Combat Idles (Taunts)
- Lost Grimoire SSE - ENB Hands Addon
- Magic Sneak Attacks
- Nirnroot Sound Replacers Collection (over 100 choices)
- Sprint Stamina Fix
- The Restless Dead Update 1.2.6.1
- Yovanna - A High Poly Head Racemenu Preset

### Removed Mods

- Edmond's Snowy Improvments for Rudys HQ Nordic Ruin Retexture
- Kauz ENB
- Kauz ENB Data Files
- Pandora_Behaviour_Engine_v2.7.0-beta
- Static Mesh Improvement Mod - Complex Parallax Addon
- TMD Epic Waterfalls

### Updated Mods

- Pandora
